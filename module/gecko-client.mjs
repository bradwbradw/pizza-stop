...
import geckoSwagger from "./gecko-swagger.json" assert { type: "json" };
...
var fields =
  "id symbol platforms categories name public_notice additional_notices description links image market_data.market_cap.usd market_data.market_cap.btc community_data.twitter_followers market_data.total_value_locked.usd market_data.total_value_locked.btc sentiment_votes_up_percentage market_cap_rank market_data.price_change_percentage_7d market_data.price_change_percentage_24h market_data.price_change_percentage_30d".split(
    " "
  );
...
var domain = process.env.DOMAIN || "localhost:5678";
if (process.env.COINGECKO_API_KEY) {
  ...
  GeckoApi = new SwaggerClient({ spec: geckoSwagger })
    .then((client) => {
      if (!client) {
        console.error("Failed to initialize GeckoApi: SwaggerClient returned undefined");
        return Promise.reject("SwaggerClient returned undefined");
      }
    ...
    .catch((err) => {
      console.error("Error initializing GeckoApi:", err);
...
// TODO convert to promise. fetch market cap and select largest
...
function geckoIDFromTicker(ticker) {
  ...
  console.log(`other options were:`,list)
  return id;
}
function aGoodId(id) {
...
function getGeckoCoins() {
  if (_.isEmpty(geckoCoins)) {
    ...
    return GeckoApi.then((g) => {
      ...
      geckoCoins = cache.setPersistent(geckoCacheKey, coins);
      return geckoCoins;
    });
  } else {
    ...
    if (_.isString(id) && id != "") {
      ...
      return GeckoApi.then((g) => {
        return new Promise((resolve, reject) => {
          ...
          setTimeout(
            () => reject(`gecko get ${id} timeout after ${timeout} ms`),
            timeout
          );
        }).then((r) => {
          ...
          .catch((err) => {
            ...
            if (err == "ratelimit" && geckoTimeout < 6000) {
              ...
              return new Promise((resolve, reject) => {
                ...
                return doGeckoRequest({ ticker }).then((r) => {
                  return cache.set(cacheKey, r);
                });
              });
            } else {
      ...
      return p.then((r) => {
        if (chainID) {
          ...
          _.each(platforms, (address, name) => {
            var pair = _.find(chainPairs, (p) => {
              return _.get(p[1], "name") == name;
            });
...
function tickerContract(chainID, ticker) {
  ...
  if (_.isString(address)) {
    ...
    return asset(ticker, chainID).then((assetData) => {
